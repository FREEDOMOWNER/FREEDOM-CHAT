...
// ===== SECURITY FIX: Client-side rate limiting trackers =====
const rateLimiters = {
    login: { attempts: 0, lastTime: 0, blockUntil: 0 },
    register: { attempts: 0, lastTime: 0, blockUntil: 0 },
    recovery: { attempts: 0, lastTime: 0, blockUntil: 0 }
};

const RATE_LIMITS = {
    login: { window: 2000, max: 5, blockTime: 300000 }, // 2 sec, 5 attempts, 5 min block
    register: { window: 5000, max: 3, blockTime: 600000 }, // 5 sec, 3 attempts, 10 min block
    recovery: { window: 10000, max: 3, blockTime: 900000 } // 10 sec, 3 attempts, 15 min block
};
...
function checkRateLimit(action) {
    ...
    if (limiter.blockUntil > now) {
        const minutesLeft = Math.ceil((limiter.blockUntil - now) / 60000);
        return {
            allowed: false,
            message: `Too many attempts. Please wait ${minutesLeft} minute${minutesLeft > 1 ? 's' : ''}.`
        };
    }

    // Check time window
    if (now - limiter.lastTime < limits.window) {
        limiter.attempts++;
    } else {
    ...
    if (limiter.attempts > limits.max) {
        limiter.blockUntil = now + limits.blockTime;
        const minutes = Math.ceil(limits.blockTime / 60000);
        return {
            allowed: false,
            message: `Too many attempts. Blocked for ${minutes} minutes.`
        };
    }
...
// Load device fingerprint library
document.addEventListener('DOMContentLoaded', function() {
    if (!window.DeviceFingerprint) {
        const script = document.createElement('script');
        script.src = 'deviceid.js';
...
function validateUsername(username) {
    if (username.length < 3 || username.length > 16) {
        return { valid: false, message: 'Username must be between 3 and 16 characters' };
    }
...
function validatePassword(password) {
    ...
    if (!/^[a-zA-Z0-9!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]+$/.test(password)) {
        return { valid: false, message: 'Password can contain letters, numbers, and symbols' };
    }
    return { valid: true };
}
...
function generateRecoveryPhrase() {
    // ===== SECURITY FIX: Using larger word list for better entropy =====
    const wordList = [
    ...
    for (let i = 0; i < 16; i++) {
        ...
        } while (usedIndices.has(randomIndex)); // Avoid duplicates for better security
...
function showNotification(message, type = 'info') {
    const existingNotification = document.querySelector('.fsc-notification');
    if (existingNotification) {
        existingNotification.remove();
    }

    const notification = document.createElement('div');
    notification.className = `fsc-notification fsc-notification-${type}`;
    ...
    switch(type) {
        ...
        case 'warning':
            ...
            break;
        case 'success':
            notification.style.background = 'rgba(20, 184, 158, 0.95)';
            notification.style.color = '#ffffff';
            notification.style.borderLeft = '4px solid #0F766E';
            break;
        default:
            notification.style.background = 'rgba(51, 65, 85, 0.95)';
            notification.style.color = '#ffffff';
    }

    if (!document.querySelector('#fsc-notification-styles')) {
...
// LOGIN FUNCTION - with rate limiting and CSRF
async function login(event) {
    ...
    const deviceId = window.DeviceFingerprint ?
        await window.DeviceFingerprint.getDeviceId() : 'unknown';

    const usernameCheck = validateUsername(originalUsername);
    if (!usernameCheck.valid) {
        showNotification(escapeHtml(usernameCheck.message), 'error');
        return;
    }

    const passwordCheck = validatePassword(password);
    if (!passwordCheck.valid) {
        showNotification(escapeHtml(passwordCheck.message), 'error');
        return;
    }
    ...
    try {
        ...
        if (data.success) {
            sessionStorage.setItem('fsc_username', originalUsername);
            if (data.role) {
                sessionStorage.setItem('fsc_role', data.role);
            }
            showNotification(`✅ Welcome back, ${escapeHtml(originalUsername)}!`, 'success');
            ...
            if (data.message.includes('locked')) {
                ...
                const minutes = minutesMatch ? minutesMatch[0] : '';

                showNotification('🔒 ' + escapeHtml(data.message), 'error');

                if (minutes) {
                    loginButton.textContent = `LOCKED (${minutes}m)`;
            ...
            } else if (data.message.includes('attempt')) {
                showNotification('⚠️ ' + escapeHtml(data.message), 'warning');
                loginButton.disabled = false;
                loginButton.textContent = originalButtonText;
...
// REGISTER FUNCTION - now uses main.js but registration is handled in register.html
// This is kept for compatibility but registration now uses the version in register.html

function showRecoveryPhraseModal(phrase, onConfirm) {
    ...
    // ===== SECURITY FIX: Escape the phrase for display =====
    const escapedPhrase = escapeHtml(phrase);

    modal.innerHTML = `
        <h2 style="color: var(--freedom-teal-light); margin-bottom: 16px; font-size: 24px;">⚠️ SAVE YOUR RECOVERY PHRASE</h2>
        <p style="color: var(--freedom-text-secondary); margin-bottom: 24px; font-size: 14px;">This is the ONLY way to recover your account. Write it down and store it safely!</p>
        <div style="background: var(--freedom-bg-tertiary); border: 1px solid var(--freedom-border); border-radius: 16px; padding: 24px; margin-bottom: 24px;">
            <p style="color: var(--freedom-teal-light); font-size: 18px; font-weight: 500; word-spacing: 8px; line-height: 1.6;">${escapedPhrase}</p>
        </div>
        <div style="display: flex; gap: 16px;">
            <button onclick="copyToClipboard('${escapedPhrase.replace(/'/g, "\\'")}')" style="flex: 1; padding: 12px; background: transparent; border: 1px solid var(--freedom-teal-light); color: var(--freedom-teal-light); border-radius: 30px; cursor: pointer; font-weight: 500;">📋 Copy</button>
            <button id="confirmPhraseBtn" style="flex: 1; padding: 12px; background: linear-gradient(135deg, var(--freedom-deep-teal) 0%, var(--freedom-teal-light) 100%); border: none; color: white; border-radius: 30px; cursor: pointer; font-weight: 500;">✅ I've Saved It</button>
        </div>
    `;
    ...
    document.getElementById('confirmPhraseBtn').onclick = () => {
...
window.copyToClipboard = function(text) {
    navigator.clipboard.writeText(text).then(() => {
        showNotification('✅ Recovery phrase copied to clipboard!', 'success');
    }).catch(() => {
